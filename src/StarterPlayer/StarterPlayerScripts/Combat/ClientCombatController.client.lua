...
local function bindToolTracking(character: Model)
	...
	character.ChildAdded:Connect(function(c)
		if c:IsA("Tool") and c.Name == CombatConfig.ToolName then
			_toolEquipped = true
		end
	end)
...
CombatEvent.OnClientEvent:Connect(function(payload: CombatTypes.CombatEventPayload)
	...
	if kind == "Swing" then
		...
		-- Only play your own swing animation locally; dummy swing anims are server-side
...
print("[Combat] ClientCombatController initialized.")